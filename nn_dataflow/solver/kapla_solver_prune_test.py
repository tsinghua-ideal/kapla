...
class KaplaSolver:
    ...
    def __init__(self, network, array_mapping, batch_size, resource, unit_cost, options, ntops=1):
        ...
        part_esti_ratio = 0.3
        part_esti_abs_max_num = 16
        explore_n_seg_sets = 4
        self.selected_segments = self.solve_segment(part_esti_ratio, part_esti_abs_max_num,
                                                    explore_n_seg_sets)
    ...
    def solve_dataflow(self):
        ...
        for layer_name in self.ordered_layer_list:
            # if layer_name == 'point_conv4':
            #     raise ValueError('Intentionaly interruptted.')
    ...
    def solve_segment(self, part_esti_ratio=1.0, part_esti_abs_max_num=32, explore_n_seg_sets=8):
    ...
    def fill_stack(self, layer_type, layer_data_size, remain_lcnt, hw_repls, bl_ord):
        ...
        # print('Start fill_stack....')
        # print('inm_ltype', inm_ltype, 'inm_irr_ds', inm_irr_ds, 'outer_irr_ds', outer_irr_ds)
        ...
        for drc in range(len(hw_repls)):
            while hw_repls[drc] != 1:
                ...
                # print('hw_repls', hw_repls[drc], 'min_factor', min_factor)
                ...
                for dce in outer_irr_ds:
                    ...
                    if any(remain_lcnt[dim] > 1 for dim in dce_irr_dims):
                        outer_trivial = False
                # print('outer_trivial', outer_trivial)
                if outer_trivial:
                    # print('remain_lcnt', remain_lcnt)
                    inm_rlvt_dims = self.tdm.get_ltype_rlvt_dims(layer_type, inm_ltype)
                ...
                # print('remain_lcnt', remain_lcnt, 'outer_refetch_size', outer_refetch_size, 'irr_ltype', irr_ltype)
                dividable = False
                ...
                if not dividable:
                    ...
                    # # TODO: Review the above code. Choose the one with least fetch data size.
                    # for irr_ltype in range(le.NUM):
                    #     for dim in self.tdm.get_ltype_rlvt_dims(layer_type, irr_ltype):
                    #         temp_fetch_size = [refetch_size[dce] for dce in range(de.NUM)]
                    #         extra_ratio = util.idivc(remain_lcnt[dim], min_factor) * min_factor / \
                    #                       remain_lcnt[dim]
                    #         for d in self.tdm.get_dim_rlvt_dtypes(layer_type, dim):
                    #             temp_fetch_size[d] * extra_ratio
                    #             if irr_ltype in outer_dim:
                    #                 temp_fetch_size[d] /= min_factor
                    #         if sum(temp_fetch_size) < min_cost:
                    #             min_cost = sum(temp_fetch_size)
                    #             min_cost_dim = dim
                    #             min_cost_dtype = dce
                    ...
                    stacks_repl_dict[drc][min_cost_dim] *= min_factor
                    hw_repls[drc] = util.idivc(hw_repls[drc], min_factor)
            # print('stacks_repl_dict', stacks_repl_dict)
        ...
        for dim in sorted_dims:
            ...
            for repl in repl_list:
                if layer_type in (lte.CONV, lte.LOCAL, lte.DW_CONV):
                    if self.array_mapping == ame.ROW_STATIONARY and dim == "Yo":
                        yo_stack_idx = None
                        for idx, stack in enumerate(base_stacks):
                            if "Yo" in stack:
                                yo_stack_idx = idx
                                break
