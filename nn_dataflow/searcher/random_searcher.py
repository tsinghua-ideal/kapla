...
class KaplaSearcher():
    ...
    def search_dataflow(self):
        ...
        for layer_name in self.ordered_layer_list:
            ...
            for seg in self.segments[layer_name]:
                ...
                constraint_handle = util.random_collect(
                    seg.gen_constraint(self.options.layer_pipeline_time_ovhd), constraint_prob)
    ...
    def search_segment_df(self, segment, allocation, constraint, cur_nndf):
        ...
        for sp_idx, (ltpl, rtpl, ctpl) in enumerate(zip(segment, allocation, constraint)):
            ...
            for tm_idx, (layer_name, resource, cstr) in enumerate(zip(ltpl, rtpl, ctpl)):
                ...
                df, cost_dict, layer_time, real_cstr, sched_vars = self.search_layer_df(layer_name, cur_cstr, resource, \
    ...
    def search_layer_df(self, layer_name, cstr, resource, sp_idx, tm_idx, ifmap_layout):
        ...
        partition_prob = 0.04
        partition_handle = util.random_collect(partition.gen_partition(layer, self.batch_size, resource.proc_region.dim,
                                                 self.options, guaranteed=True), partition_prob)
        ...
        for part in partition_handle:
            ...
            r = apply_func(_search_layer_df_perprocess, (layer_type, conv_strds,
                           ...
                           resource, cstr, self.array_mapping, unit_nhops, self.unit_cost, self.options))
            results.append(r)
...
def _search_layer_df_perprocess(layer_type, conv_strds, froz_parted_workload, part, buf_sharing,
    ...
    for loopcnt, unit_size, logic_region, regf_stack, origin_regf_update, unit_ops, regf_repls in mapping.gen_array_mapping():
        ...
        bl_ts_prob = 0.1
        lb_handle = util.random_collect(generate_loop_blocking(loopcnt, constraint), bl_ts_prob)
        for knobs_tuple, bl_ts, real_cstr in lb_handle:
            ...
            bl_ord_counter = 0
            bl_ord_prob = 0.1
            bl_ord_handle = util.random_collect(itertools.product(*[itertools.permutations(range(len(knobs_tuple))) for _ in range(BL.NUM)]), bl_ord_prob)
            for bl_ords in bl_ord_handle:
                ...
                # nndf_lbs, nndf_cost = compare_with_nn_dataflow(layer_type, mapping, gbuf_workload, conv_strds, loopcnt, unit_size,
                #                                      knobs_tuple, bl_ts, bl_ords, unit_ops,
                #                                      resource, buf_sharing, unit_cost, options)
                # pp.pprint(nndf_cost)
                # print("")
                ...
                if total_cost < min_total_cost:
                    ...
                    min_layer_vars = (layer_type, logic_region, mapping_fold, mapping_repls, part, conv_strds, loopcnt, unit_size,
                        knobs_tuple, bl_ts, bl_ords, unit_ops, resource, buf_sharing, accesses_result)
                    min_layer_df = layer_rearrange(tdm, layer_type, gbuf_tensor_dims, gbuf_stack, sorted_gbuf_updates, regf_tensor_dims, regf_stack,
